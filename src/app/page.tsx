...
export default function Homepage() {
  ...
  return (
    <div className="bg-[#FFF9F2] text-[#1A1A1A] min-h-screen relative overflow-x-hidden font-sans">
      ...
      {/* Hero Section with Purple Wavy Gradient */}
      <section className="relative pt-36 pb-24 md:pt-48 md:pb-36 bg-gradient-to-br from-[#8A2BE2] via-[#4D2677] to-[#2A1147] text-white overflow-hidden">
        ...
        <div
          className="absolute top-24 left-10 w-20 h-20 pointer-events-none transition-transform duration-300 opacity-60 hidden md:block animate-float-slow"
          style={{ transform: `translate(${mousePos.x * -0.5}px, ${mousePos.y * -0.5}px)` }}
        >
          <Leaf className="w-full h-full text-brand-green fill-brand-green/20 rotate-12" />
        </div>
        <div
          className="absolute bottom-20 left-1/3 w-16 h-16 pointer-events-none transition-transform duration-300 opacity-40 hidden md:block animate-float-reverse"
        ...
        <div className="max-w-container mx-auto px-6 md:px-12 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
          ...
          <div className="lg:col-span-6 space-y-6 text-left">
            ...
            <div className="flex flex-wrap items-center gap-4 pt-2">
              <Link
                href="/shop"
                className="px-8 py-4 bg-[#FFC531] hover:bg-[#FF9F1C] text-[#2A1147] font-black text-sm uppercase tracking-widest rounded-full transition-all duration-300 shadow-md hover:shadow-lg flex items-center gap-2"
              >
          ...
          {/* Right Product Composition with Floating Badges */}
          <div className="lg:col-span-6 flex justify-center relative pt-10 lg:pt-0">
            {/* Main composition container */}
            <div 
              className="relative w-72 h-72 md:w-[480px] md:h-[400px] transition-transform duration-300 flex items-center justify-center"
              style={{ transform: `translate(${mousePos.x * 0.4}px, ${mousePos.y * 0.4}px)` }}
            >
              {/* Star stamp badge rotating */}
              <div className="absolute top-0 right-4 md:right-10 w-24 h-24 bg-[#FFC531] text-[#2A1147] rounded-full p-2 flex flex-col items-center justify-center font-black text-[9px] uppercase tracking-wider animate-spin-slow shadow-lg border-4 border-[#2A1147] z-20">
                <span>★ 100% VEGAN ★</span>
                <span className="text-[7px]">Certified Organic</span>
              </div>

              {/* Star-burst minerals badge */}
              <div className="absolute top-24 left-0 bg-[#FF9F1C] text-white px-3.5 py-1.5 rounded-full font-black text-[10px] uppercase tracking-widest shadow-md rotate-[-8deg] z-20 animate-bounce">
                Essential Minerals
              </div>

              {/* Lower Sugar bubble */}
              <div className="absolute bottom-12 left-10 bg-[#E9418A] text-white px-4 py-2 rounded-full font-black text-[10px] uppercase tracking-widest shadow-md rotate-[12deg] z-20 animate-pulse">
                Lower Sugar
              </div>
              ...
              {/* Sourced Product Mockups overlapping */}
              <div className="relative w-full h-full flex items-center justify-center">
                ...
                <img
                  ...
                  alt="Açaí pure packs"
                  className="w-36 h-36 md:w-48 md:h-48 object-contain drop-shadow-2xl absolute right-0 bottom-4 z-10 transform rotate-12"
                />
                ...
                <img
                  ...
                  alt="Smoothie Cup"
                  className="w-32 h-32 md:w-40 md:h-40 object-contain drop-shadow-2xl absolute left-0 bottom-0 z-20 transform rotate-[-12deg]"
                />
              </div>
            </div>
          </div>
      ...
      {/* Section: SQUEEZE IN SOME GOODNESS */}
      <section className="py-24 max-w-container mx-auto px-6 md:px-12 text-center space-y-16 relative">
        {/* Floating Fruit Assets on sides */}
        <div className="absolute top-10 left-4 w-12 h-12 opacity-60 hidden md:block animate-float">
          {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div className="absolute bottom-10 right-4 w-14 h-14 opacity-60 hidden md:block animate-float-reverse">
          {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div className="space-y-4 max-w-2xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-black text-[#2A1147] uppercase tracking-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            ...
            <div
              key={item.title}
              className={`bg-white border p-8 rounded-card text-left space-y-4 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group ${item.color.split(" ")[2]}`}
            >
      ...
      <section className="py-24 bg-gradient-to-r from-[#FF9F1C] to-[#FFC531] text-white relative">
          ...
          <div className="lg:col-span-7 relative flex items-center gap-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 overflow-hidden w-full py-4">
              {favouritesList.slice(favIndex, favIndex + 2).concat(favouritesList).slice(0, 3).map((item, idx) => (
                <div
                  key={idx}
                  className="bg-white text-brand-purple rounded-card p-5 border border-brand-purple/5 shadow-md flex flex-col justify-between text-center aspect-[3/4] group transition-all duration-500 hover:scale-103"
                >
                  <div className="space-y-4">
      ...
      <section className="py-24 max-w-container mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center text-left">
        ...
        {/* Left Copy */}
        <div className="lg:col-span-5 space-y-6">
          <h2 className="text-4xl md:text-5xl font-black text-[#2A1147] leading-none uppercase">
            WILDLY FLAVOURFUL, <br />
            TOTALLY ADDICTIVE. <br />
            PACKED WITH TASTE
          </h2>
          <div className="inline-block bg-[#FFC531] text-[#2A1147] font-black text-xl px-4 py-2 rounded-btn shadow-sm rotate-[-2deg]">
            YOUR NEW FROZEN CRUSH
          </div>
          <p className="text-brand-purple/90 text-sm md:text-base font-black uppercase tracking-wider">
        ...
        <div className="lg:col-span-4 relative group">
          ...
          <div className="rounded-card overflow-hidden border border-brand-purple/5 aspect-[3/4] shadow-lg relative">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img 
        ...
        <div className="lg:col-span-3 space-y-4">
          {[
            ...
            <div 
              key={idx}
              className={`p-4 rounded-full flex items-center justify-center text-white text-sm font-black uppercase tracking-widest shadow-md ${pill.color}`}
            >
              {pill.title}
            </div>
          ))}
      ...
      {/* Find Your Routine (Horizontal wrapping categories section) */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {recipes.slice(0, 5).map((recipe) => (
            <div
              ...
              className="bg-white rounded-card overflow-hidden border border-[#2A1147]/5 shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer group"
            >
      ...
      <section className="py-20 max-w-container mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-3 gap-8">
        
        {/* Card 1: School Days */}
        <div className="bg-[#e9f3eb] rounded-card border border-brand-green/10 overflow-hidden flex flex-col justify-between shadow-sm hover:shadow-md transition-all text-left">
        ...
        <div className="bg-brand-purple text-white rounded-card overflow-hidden flex flex-col justify-between shadow-sm hover:shadow-md transition-all text-left">
          <div className="p-8 space-y-6">
