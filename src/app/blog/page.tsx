    ...
    <div className="bg-brand-ivory text-brand-charcoal min-h-screen relative overflow-x-hidden font-sans">
      ...
      <section className="py-12 max-w-container mx-auto px-6 md:px-12 space-y-12 text-left">
        ...
        {selectedCategory === "All" && !searchQuery && featured && (
          <div className="bg-white rounded-card overflow-hidden border border-brand-charcoal/5 shadow-sm hover:shadow-xl transition-all duration-300 grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch group">
            <div className="lg:col-span-7 bg-brand-ivory overflow-hidden min-h-64 relative aspect-[16/10] lg:aspect-auto">
            ...
            <div className="lg:col-span-5 p-8 flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                ...
                <Link href={`/blog/${featured.id}`} className="block">
                  <h2 className="text-2xl md:text-3xl font-bold font-display text-brand-charcoal leading-tight hover:text-brand-orange transition-colors">
                    {featured.title}
                  </h2>
                </Link>
                <p className="text-brand-charcoal/75 text-sm font-medium leading-relaxed">
                  {featured.excerpt}
                </p>
              </div>

              <div className="flex items-center gap-6 text-xs text-brand-charcoal/50 font-semibold pt-4 border-t border-brand-charcoal/5">
        ...
        {filteredBlogs.length === 0 ? (
          <div className="text-center py-20 bg-white border border-brand-charcoal/5 rounded-card space-y-4">
            <p className="text-lg font-bold text-brand-charcoal">No articles found</p>
            <p className="text-sm text-brand-charcoal/50">Try modifying your filters or search keywords.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {regularPosts.map((post) => (
              <div
                key={post.id}
                className="bg-white border border-brand-charcoal/5 rounded-card overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group"
              >
                <div className="space-y-4">
                  <div className="aspect-[16/10] w-full bg-brand-ivory overflow-hidden relative">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={post.image}
                      alt={post.title}
                  ...
                  <div className="p-6 space-y-3">
                    ...
                    <Link href={`/blog/${post.id}`} className="block">
                      <h4 className="text-xl font-bold font-display text-brand-charcoal hover:text-brand-orange transition-colors leading-snug">
                        {post.title}
                      </h4>
                    </Link>
                    <p className="text-brand-charcoal/75 text-xs font-medium leading-relaxed">
                      {post.excerpt}
                    </p>
                  </div>
                </div>

                <div className="p-6 pt-4 border-t border-brand-charcoal/5 flex items-center justify-between text-[11px] font-bold text-brand-charcoal/50">
                  <span className="flex items-center gap-1">
