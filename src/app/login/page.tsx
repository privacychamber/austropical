...
const AppleIcon = () => (
  <svg className="w-4 h-4 text-brand-charcoal" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div className="bg-brand-ivory text-brand-charcoal min-h-screen relative overflow-x-hidden font-sans">
      ...
      <section className="pt-32 pb-24 max-w-container mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="hidden lg:block lg:col-span-6 space-y-6 text-left">
          <div className="inline-flex items-center gap-2 bg-brand-charcoal/5 px-4 py-2 rounded-btn">
            <ShieldCheck className="w-4 h-4 text-brand-orange" />
            <span className="text-xs font-black uppercase tracking-wider">Secure Access</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-black font-display text-brand-charcoal leading-tight">
            Nourish Your Journey.
          </h2>
          <p className="text-brand-charcoal/80 text-base font-semibold max-w-md">
          ...
          <img
            ...
            alt="Smoothie"
            className="w-full h-48 object-cover rounded-card border border-brand-charcoal/5 shadow-md"
          />
        </div>

        {/* Right Side: Account details form card */}
        <div className="lg:col-span-6 max-w-md mx-auto w-full">
          <div className="bg-white border border-brand-charcoal/5 p-8 md:p-10 rounded-card shadow-lg text-left space-y-6">
              ...
              {step === 1 && (
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-brand-charcoal/50 uppercase tracking-widest block">
                      Email Address
                    </label>
                    <div className="relative">
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-2.5 bg-brand-ivory border border-brand-charcoal/10 rounded-input text-sm font-semibold outline-none focus:border-brand-charcoal"
                      />
                      <Mail className="absolute left-3.5 top-3 w-4.5 h-4.5 text-brand-charcoal/40" />
                    </div>
                  </div>
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-3.5 bg-brand-charcoal hover:bg-brand-charcoal-light text-white font-bold rounded-btn transition-colors flex items-center justify-center gap-2"
                  >
                    {loading ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full py-3 bg-brand-ivory border border-brand-charcoal/10 rounded-input text-center text-xl font-bold tracking-widest outline-none focus:border-brand-charcoal"
                    />
                  </div>
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-3.5 bg-brand-orange hover:bg-brand-yellow text-brand-charcoal font-black rounded-btn transition-colors flex items-center justify-center gap-2"
                  >
                    {loading ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
                    ) : (
                  ...
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="text-xs text-brand-charcoal/50 font-bold block text-center w-full hover:underline"
                  >
                    Back to email input
                  </button>
                </div>
            ...
            {step === 1 && (
              <div className="space-y-4 pt-4 border-t border-brand-charcoal/10">
                <div className="relative text-center">
                  <span className="bg-white px-3 text-xs font-semibold text-brand-charcoal/40 relative z-10">
                    Or secure credentials via
                  </span>
                  <div className="absolute top-1/2 left-0 right-0 h-px bg-brand-charcoal/10 z-0" />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() => handleSocialMock("Google")}
                    className="py-2.5 border border-brand-charcoal/10 rounded-btn hover:bg-brand-ivory text-xs font-bold flex items-center justify-center gap-2 transition-colors"
                  >
                    <GoogleIcon />
                    <span>Google</span>
                  </button>
            ...
            {/* Bottom toggle */}
            <div className="pt-2 text-center text-xs font-semibold text-brand-charcoal/60">
              <button
                onClick={() => {
                  setIsLogin(!isLogin);
                  setStep(1);
                }}
                className="hover:underline text-brand-orange font-bold"
              >
